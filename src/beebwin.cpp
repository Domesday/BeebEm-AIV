...
// Extra variables to allow the scaling and mixing of an image from the LV-rom
// (i.e. The Video disc player used in the BBC Domesday system) [DMS 20/11/02]
HDC hdcMem;
HBITMAP hbmMem;
bmiData lvBmi;

// The picture mixing mode for combining images from the LV-ROM and the BBC
...
static const char *WindowTitle =
			"BBC Master 128 Domesday System Emulator";
static const char *AboutText = "BBCDomesEm\n"
			"BBC Master 128 Domesday System Emulator\n"
			"Version 1.5, 8th December 2002";

/* Configuration file strings */
// static const char *CFG_FILE_NAME = "BeebEm.ini";
static const char *CFG_FILE_NAME = "DomesEm.ini";
...
{   
  ...
  {
    m_isFullScreen = GetPrivateProfileInt(CFG_VIEW_SECTION, "isFullScreen",
    						0, CFG_FILE_NAME);
  }

  m_DDFullScreenMode = GetPrivateProfileInt(CFG_VIEW_SECTION,
  			"DDFullScreenMode", ID_VIEW_DD_640X480, CFG_FILE_NAME);
  m_isDD32           = GetPrivateProfileInt(CFG_VIEW_SECTION, "isDD32",
  						0, CFG_FILE_NAME);
  ...
  GetPrivateProfileString(CFG_OPTIONS_SECTION, CFG_OPTIONS_KEY_MAPPING,
  		DefValue, CfgValue, sizeof(CfgValue), CFG_FILE_NAME);
  m_MenuIdKeyMapping = atoi(CfgValue);
  TranslateKeyMapping();

  GetPrivateProfileString(CFG_AMX_SECTION, CFG_AMX_ENABLED,
  		"0", CfgValue, sizeof(CfgValue), CFG_FILE_NAME);
  AMXMouseEnabled = atoi(CfgValue);

  GetPrivateProfileString(CFG_AMX_SECTION, CFG_AMX_LRFORMIDDLE,
  ...
  *binsize=1;
  RegRes=SysReg.GetBinaryValue(HKEY_CURRENT_USER, "Software\\BeebEm",
				"MachineType", &MachineType, binsize);
  if (!RegRes)
  {
    MachineType=0;
    SysReg.CreateKey(HKEY_CURRENT_USER, "Software\\BeebEm");
    SysReg.SetBinaryValue(HKEY_CURRENT_USER,"Software\\BeebEm",
				"MachineType", &MachineType, binsize);
  }
  RegRes=SysReg.GetBinaryValue(HKEY_CURRENT_USER, "Software\\BeebEm",
				"RelaySoundEnabled",&RelaySoundEnabled,binsize);
  if (!RegRes)
  {
    RelaySoundEnabled=0;
    SysReg.SetBinaryValue(HKEY_CURRENT_USER, "Software\\BeebEm",
  ...
  RegRes=SysReg.GetBinaryValue(HKEY_CURRENT_USER, "Software\\BeebEm",
				"TubeEnabled", &TubeEnabled, binsize);
  if (!RegRes)
  {
    TubeEnabled=0;
    SysReg.SetBinaryValue(HKEY_CURRENT_USER, "Software\\BeebEm",
  ...
  RegRes=SysReg.GetBinaryValue(HKEY_CURRENT_USER, "Software\\BeebEm",
				"TapeSoundEnabled", &TapeSoundEnabled, binsize);
  if (!RegRes)
  {
  ...
  *binsize=2;
  RegRes=SysReg.GetBinaryValue(HKEY_CURRENT_USER, "Software\\BeebEm",
				"Tape Clock Speed", &TapeClockSpeed, binsize);
  if (!RegRes)
  {
    TapeClockSpeed=5600;
    SysReg.SetBinaryValue(HKEY_CURRENT_USER, "Software\\BeebEm",
...
//		SoundInit();
  ...
  *binsize=sizeof(tWindowPlacement);
  RegRes=SysReg.GetBinaryValue(HKEY_CURRENT_USER, "Software\\BeebEm",
  				"WindowPos", &tWindowPlacement, binsize);
  if (!RegRes)
  {
...
void BeebWin::CreateBitmap()
{
  ...
  m_bmi.bmiHeader.biWidth = LVSCALE_IM_W;	//  640;
  m_bmi.bmiHeader.biHeight = 0 - LVSCALE_IM_H;	// -512;
  m_bmi.bmiHeader.biPlanes = 1;
  m_bmi.bmiHeader.biBitCount = 32;
  m_bmi.bmiHeader.biCompression = BI_RGB;
  m_bmi.bmiHeader.biSizeImage = LVSCALE_IM_W*LVSCALE_IM_H*BYTE_WIDTH;// 640*512*4;
  m_bmi.bmiHeader.biClrUsed = 0;
  m_bmi.bmiHeader.biClrImportant = 0;

  lvBmi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
  lvBmi.bmiHeader.biWidth = LVSCALE_IM_W;
  lvBmi.bmiHeader.biHeight = -LVSCALE_IM_H;
  lvBmi.bmiHeader.biPlanes = 1;
  lvBmi.bmiHeader.biBitCount = 24;
  lvBmi.bmiHeader.biCompression = BI_RGB;
  lvBmi.bmiHeader.biSizeImage = LVSCALE_IM_W * LVSCALE_IM_H * LVRAW_BWIDTH;
  lvBmi.bmiHeader.biClrUsed = 0;
  lvBmi.bmiHeader.biClrImportant = 0;
...
//m_bmi.bmiColors=NULL;
  m_hBitmap = CreateDIBSection(m_hDCBitmap, (BITMAPINFO *)&m_bmi,
				DIB_PAL_COLORS, (void**)&m_screen, NULL,0);
#else
  for (int i = 0; i < 9; ++i)
  {
    float r,g,b;
  ...
  m_hBitmap = CreateDIBSection(m_hDCBitmap, (BITMAPINFO *)&m_bmi,
  				DIB_RGB_COLORS, (void**)&m_screen, NULL, 0);
//m_bmi.bmiColors=NULL;
#endif
...
void BeebWin::CreateBeebWindow(void)
{
  ...
  m_hWnd = CreateWindow( "BEEBWIN",	// See RegisterClass() call.
			  ...
			  NULL);	// We don't have data in our WM_CREATE
...
void BeebWin::InitMenu(void)
{
  ...
  CheckMenuItem(hMenu, IDM_ALLOWALLROMWRITES,
  			WritableRoms ? MF_CHECKED : MF_UNCHECKED); 
  ...
  ModifyMenu(hMenu, IDM_PRINTER_FILE, MF_BYCOMMAND,
  			IDM_PRINTER_FILE, menu_string);
...
HRESULT BeebWin::InitSurfaces(void)
{
  ...
  if (m_isFullScreen)
  {
    ddrval = m_DD2->SetCooperativeLevel(m_hWnd,
  ...
  if (ddrval == DD_OK)
  {
    ...
    ddsd.dwWidth = LVSCALE_IM_W;
    ddsd.dwHeight = LVSCALE_IM_H;
...
void BeebWin::SetRomMenu(void)
{
  ...
  for( i=0; i<16; i++ )
  {
    ...
    ModifyMenu( hMenu,			// handle of menu 
		...
		IDM_ALLOWWRITES_ROM0 + i,	// menu item id or pop-up menuh
		Title);			// menu item content 

  /* LRW Now uncheck Roms which are NOT writable, that are already loaded. */
...
void BeebWin::GetRomMenu(void)
{
  ...
  /* LRW Now uncheck the Roms as NOT writable, that have already been loaded. */
    RomWritable[i] = ( GetMenuState(hMenu, IDM_ALLOWWRITES_ROM0+i, MF_BYCOMMAND)
    					& MF_CHECKED );
  }
}
...
void BeebWin::GreyRomMenu(BOOL SetToGrey)
{
  ...
  for (int i=1; i<16; ++i)
  {
    EnableMenuItem(hMenu, IDM_ALLOWWRITES_ROM0 + i,
    			SetToGrey ? MF_GRAYED : MF_ENABLED );
  }
}
			...
			LPARAM lParam)	   // additional information
{
  ...
  switch (message)
  {
    case WM_CREATE:
    {
      ...
      hbmMem = CreateCompatibleBitmap(hdc, LVSCALE_IM_W, LVSCALE_IM_H);
...
//      {
//        char dbg_msg[80];
//	sprintf(dbg_msg, "Key %d %d pressed\n", row, col);
//        MessageBox(GETHWND, dbg_msg, WindowTitle, MB_OK|MB_ICONERROR); 
//      }
      break;
	...
	//SoundChipReset();
      ...
      {
	if (col==-3)
	  SoundTuning+=0.1; // Page Up
	if (col==-4)
	  SoundTuning-=0.1; // Page Down
      }
      break;					  
...
// Experiment: show menu in full screen when cursor moved to top of window
    ...
    case WM_RBUTTONDOWN:
      // Simulate using the Change button in the Domesday system track ball
      // by detecting a Right Mouse click and interpreting that as pressing
      // the TAB key. (This is a fudge, so the trackball will not work
      // properly with other software on the BBC - just with Domesday.) [DMS]
      BeebKeyDown(6, 0);
...
int BeebWin::StartOfFrame(void)
{
	...
	if (UpdateTiming()) {
		...
		memset(m_screen,0,640*((TeletextEnabled)?500:256));
	}
...
void BeebWin::updateLines(HDC hDC, int starty, int nlines)
{
  ...
  int TextStart=236; 
  unsigned char BVD;
  unsigned long FVD,FVD_nl,VVD,VVD_nl,CVD;
  long vscrptr, vscrp_nl, vBBCptr, vLVptr, vLVp_nl, vsp_inc, vLVp_inc, xcnt=0;
  char dbg_msg[80];
  ...
  // Handle merging the BBC video output with the LV-Rom output.

  if (m_screen!=NULL)
  {
    // BVD = single byte (just lower 3 bits) for BBC video
    // CVD = 3 Byte (RGB) for BBC video (calc from BVD)
    // VVD = 3 Byte (RGB) for LV-Rom video
    // FVD = 3 Byte (RGB) mixed BBC and LV-Rom video
    // VVD_nl = Source LV-Rom video (next line)
    // FVD_nl = mixed video (next line)

    vscrptr = 0;
    vLVptr = 0;

    if (m_MenuIdWinSize == IDM_LVSIZE)
    {

// if (m_DirectDrawEnabled)
// {
//   sprintf(dbg_msg, "DD enabled, IDM_LVSIZE, mixmode %d", MixMode);
//   MessageBox(GETHWND, dbg_msg, WindowTitle, MB_OK|MB_ICONERROR); 
// }

      // Use a differnt algorithm for each mixing mode.
      switch (MixMode)
      {
        case MIXMODE_LV:
	{
	  // Because the playing of video sequences is too slow, this has
	  // MIXMODE handles the update of the display directly and skips
	  // the rest of the function.
/*****	  BitBlt(hDC, 0, 0, LVSCALE_IM_W, LVSCALE_IM_H,
 *****		hdcMem, 0, 0, SRCCOPY);
 *****	  return;
 *****/

	  // This next section could be replaced by a much quicker GDI call
	  // see commentd code above

	  if (LVE)
	  {
	    // Duplicate the entire LV image into screen memory
	    vLVptr = 0;
	    for (vscrptr=0;
		  vscrptr < (LVSCALE_IM_W*LVSCALE_IM_H*BYTE_WIDTH);
		  vscrptr += BYTE_WIDTH, vLVptr += LVRAW_BWIDTH)
	    {
	      *(m_screen+vscrptr)   = (*(lvScaleBuff+vLVptr));
	      *(m_screen+vscrptr+1) = (*(lvScaleBuff+vLVptr+1));
	      *(m_screen+vscrptr+2) = (*(lvScaleBuff+vLVptr+2));
	    }
	  }
	  else
	  {
	    memset(m_screen, 0x00, (LVSCALE_IM_W*LVSCALE_IM_H*BYTE_WIDTH));
	  }

	} break;
	case MIXMODE_BBC:
	{
	  // blank the screen memory
	  memset(m_screen, 0x00, (LVSCALE_IM_W*LVSCALE_IM_H*BYTE_WIDTH));

	  xcnt = 0;
	  vscrptr = ((LVSCALE_IM_W*BBC_IM_YOFF) + BBC_IM_XOFF) * BYTE_WIDTH;
	  vscrp_nl = vscrptr + (LVSCALE_IM_W * BYTE_WIDTH);
	  vsp_inc = (LVSCALE_IM_W - BBC_IM_W) * BYTE_WIDTH;

          // fill in the middle section of the screen from the BBC memory
	  for (vBBCptr=0;
	  	vBBCptr < (BBC_IM_W*BBC_IM_H);
		vBBCptr++, xcnt++, vscrptr+=BYTE_WIDTH, vscrp_nl+=BYTE_WIDTH)
          {
	    if (xcnt >= BBC_IM_W)
	    {
	      xcnt = 0;
	      vscrp_nl += vsp_inc;
	      vscrptr = vscrp_nl;
	      vscrp_nl += (LVSCALE_IM_W * BYTE_WIDTH);
            }
	    BVD = *((char *)beebscreen+vBBCptr);
	    CVD = ((BVD&1)*0xff0000) |
	    		(((BVD&2)>>1)*0x00ff00) |
			(((BVD&4)>>2)*0x0000ff);

	    *(m_screen+vscrptr)   = CVD & 0xff;
	    *(m_screen+vscrptr+1) = (CVD >> 8) & 0xff;
	    *(m_screen+vscrptr+2) = (CVD >> 16) & 0xff;

	    *(m_screen+vscrp_nl)   = CVD & 0xff;
	    *(m_screen+vscrp_nl+1) = (CVD >> 8) & 0xff;
	    *(m_screen+vscrp_nl+2) = (CVD >> 16) & 0xff;
	  }
	} break;
        case MIXMODE_HK:
	{
// Quicker ? 
//          BitBlt(m_hDCBitmap, 0, 0, LVSCALE_IM_W, LVSCALE_IM_H,
//		hdcMem, 0, 0, SRCCOPY);

	  if (LVE)
	  {
	    // Comment out this loop and introduce the BitBlt for speed if needed
	    // First copy the whole of the LV image to the screen.
	    vLVptr = 0;
	    for (vscrptr=0;
		  vscrptr < (LVSCALE_IM_W*LVSCALE_IM_H*BYTE_WIDTH);
		  vscrptr += BYTE_WIDTH, vLVptr += LVRAW_BWIDTH)
	  ...
	  else
	  {
	    memset(m_screen, 0x00, (LVSCALE_IM_W*LVSCALE_IM_H*BYTE_WIDTH));
          ...
          // Scan through the BBC screen and update the appropriate pixels
	  xcnt = 0;
	  vscrptr = ((LVSCALE_IM_W*BBC_IM_YOFF) + BBC_IM_XOFF) * BYTE_WIDTH;
	  vscrp_nl = vscrptr + (LVSCALE_IM_W * BYTE_WIDTH);
	  vsp_inc = (LVSCALE_IM_W - BBC_IM_W) * BYTE_WIDTH;
        ...
        case MIXMODE_MIX:
	{
	  if (LVE)
	  {
	    // Copy the LV video at half intensity to the screen
	    vLVptr = 0;
	    for (vscrptr=0;
		  vscrptr < (LVSCALE_IM_W*LVSCALE_IM_H*BYTE_WIDTH);
		  vscrptr += BYTE_WIDTH, vLVptr += LVRAW_BWIDTH)
	    {
	      *(m_screen+vscrptr)   = (*(lvScaleBuff+vLVptr)) >> 1;
	      *(m_screen+vscrptr+1) = (*(lvScaleBuff+vLVptr+1)) >> 1;
	      *(m_screen+vscrptr+2) = (*(lvScaleBuff+vLVptr+2)) >> 1;
	    }
	  }
	  else
	  {
	    memset(m_screen, 0x00, (LVSCALE_IM_W*LVSCALE_IM_H*BYTE_WIDTH));
          ...
          xcnt = 0;
	  vscrptr  = (LVSCALE_IM_W * BBC_IM_YOFF) * BYTE_WIDTH;
	  vscrptr += (BBC_IM_XOFF)*BYTE_WIDTH;
	  vscrp_nl = vscrptr + (LVSCALE_IM_W * BYTE_WIDTH);
	  vsp_inc = (LVSCALE_IM_W - BBC_IM_W) * BYTE_WIDTH;

          // mix in the middle section of the screen from the BBC memory
	  for (vBBCptr=0;
	  	vBBCptr < (BBC_IM_W*BBC_IM_H);
		vBBCptr++, xcnt++, vscrptr+=BYTE_WIDTH, vscrp_nl+=BYTE_WIDTH)
	  {
        ...
        case MIXMODE_ENH:
	{
	  ...
	  vsp_inc = (LVSCALE_IM_W - BBC_IM_W) * BYTE_WIDTH;
          ...
          vLVptr  = (LVSCALE_IM_W * BBC_IM_YOFF) * LVRAW_BWIDTH;
	  vLVptr += (BBC_IM_XOFF * LVRAW_BWIDTH);
          vLVp_nl = vLVptr + (LVSCALE_IM_W * LVRAW_BWIDTH);
	  vLVp_inc = (LVSCALE_IM_W - BBC_IM_W) * LVRAW_BWIDTH;

	  // if the BBC pixel is set, then use the true LV pixel value
	  for (vBBCptr=0;
	  	vBBCptr < (BBC_IM_W*BBC_IM_H);
		vBBCptr++, xcnt++, vscrptr+=BYTE_WIDTH, vscrp_nl+=BYTE_WIDTH,
			vLVptr+=LVRAW_BWIDTH, vLVp_nl+=LVRAW_BWIDTH)
	  {
	    if (xcnt >= BBC_IM_W)
	    {
	      xcnt = 0;
	      ...
	      vLVp_nl += vLVp_inc;
	      vLVptr = vLVp_nl;
	      vLVp_nl += (LVSCALE_IM_W * LVRAW_BWIDTH);
            }
	    BVD = *((char *)beebscreen+vBBCptr);
            ...
            if (BVD != 0)
	    {
	      ...
	      *(m_screen+vscrp_nl)  = (*(lvScaleBuff+vLVp_nl));
	      *(m_screen+vscrp_nl+1)= (*(lvScaleBuff+vLVp_nl+1));
	      *(m_screen+vscrp_nl+2)= (*(lvScaleBuff+vLVp_nl+2));
	    }
	  }
	} break;
      }
    ...
    {
      memset(m_screen, 0x70, (LVSCALE_IM_W*LVSCALE_IM_H*BYTE_WIDTH));
    }
  }
  ...
  if (!m_DirectDrawEnabled)
  {
    if (m_MenuIdWinSize == IDM_LVSIZE)
    {
      int win_starty = starty + BBC_IM_YOFF;
      int win_nlines = BBC_IM_H;

      if ( (DisplayCycles > 0) && (hFDCBoard != NULL) )
      {
        SetBkMode(hDC, TRANSPARENT);
	SetTextColor(hDC, 0x808080);
	TextOut(hDC, 0, TextStart,
		ExtBoard.BoardName, strlen(ExtBoard.BoardName));
      }
      if (TeletextEnabled)
      {
        // We are scuppered because the teletext assumes
	// a different BBC screen width than the other modes
	MessageBox(GETHWND, "Teletext is bad! [DMS] not handled correctly",
			WindowTitle, MB_OK|MB_ICONERROR); 
      }
    ...
    else if ((m_MenuIdWinSize == IDM_640X256) && (!TeletextEnabled))
    {
      BitBlt(hDC, 0, starty, 640, (TeletextEnabled)?TTLines:256,
	    m_hDCBitmap, 0, starty, SRCCOPY);
    }
    else
    {
	...
	TextOut(hDC,0,TextStart,ExtBoard.BoardName,strlen(ExtBoard.BoardName));
      ...
      StretchBlt(hDC, 0, win_starty, m_XWinSize, win_nlines,
	      m_hDCBitmap, 0, starty, (TeletextEnabled)?552:640,
	      			(TeletextEnabled==1)?TTLines:256, SRCCOPY);
    }
  }
  else
  {
    ...
    if (ddrval == DD_OK)
    {
      if (m_MenuIdWinSize == IDM_LVSIZE)
      {
        if (starty != 0)
	{
	  sprintf(dbg_msg, "starty != 0 (== %d)", starty);
	  MessageBox(GETHWND, dbg_msg, WindowTitle, MB_OK|MB_ICONERROR); 
	}
	BitBlt(hdc, 0, 0, LVSCALE_IM_W, LVSCALE_IM_H,
		m_hDCBitmap, 0, 0, SRCCOPY);
	...
	TextOut(hdc,0,TextStart,ExtBoard.BoardName,strlen(ExtBoard.BoardName));
      ...
      {
	char fps[50];
	sprintf(fps, "%2.2f %2d %04X %04X", m_RelativeSpeed,
		(int)m_FramesPerSecond,ProgramCounter,TubeProgramCounter);
	SetBkMode(hdc,TRANSPARENT);
	SetTextColor(hdc,0x808080);
	TextOut(hdc,380,TextStart,fps,strlen(fps));
	sprintf(fps,"%d",CFrame);
	TextOut(hdc,0,TextStart,fps,strlen(fps));
      }
      ...
      srcRect.left = 0;
      srcRect.top = 0;
      srcRect.right = LVSCALE_IM_W - 1;
      srcRect.bottom = LVSCALE_IM_H - 1; 
      // srcRect.right = (TeletextEnabled)?552:640;
      // srcRect.bottom = (TeletextEnabled)?TTLines:256; 
      
      if (!m_PageFlipping)
      {
	ddrval = m_DDS2Primary->Blt(&destRect, m_DDS2One, &srcRect,
						DDBLT_ASYNC, NULL);
	if (ddrval == DDERR_SURFACELOST)
	{
      ...
      {
	ddrval = m_BackBuffer2->Blt( &destRect, m_DDS2One, &srcRect,
					DDBLT_ASYNC, NULL);
	if (ddrval == DDERR_SURFACELOST)
	{
...
} // end of : void BeebWin::updateLines(HDC hDC, int starty, int nlines)
...
void BeebWin::TranslateWindowSize(void)
{
  if (m_isFullScreen)
  {
	...
	case ID_VIEW_DD_640X480:
	  m_XWinSize = 640; m_YWinSize = 480;
	  break;
	case ID_VIEW_DD_1024X768:
	  m_XWinSize = 1024; m_YWinSize = 768;
	  break;
	case ID_VIEW_DD_1280X1024:
	  m_XWinSize = 1280; m_YWinSize = 1024;
	  break;
      }
//  	  m_YWinSize -= GetSystemMetrics(SM_CYMENUSIZE);
  ...
  else
  {
    ...
    {
      case IDM_LVSIZE:
	m_XWinSize = LVSCALE_IM_W; m_YWinSize = LVSCALE_IM_H;
	break;
      case IDM_160X128:
	m_XWinSize = 160; m_YWinSize = 128;
	break;
      case IDM_240X192:
	m_XWinSize = 240; m_YWinSize = 192;
	break;
      case IDM_640X256:
	m_XWinSize = 640; m_YWinSize = 256;
	break;
      case IDM_320X256:
	m_XWinSize = 320; m_YWinSize = 256;
	break;
      case IDM_640X512:
	m_XWinSize = 640; m_YWinSize = 512;
	break;
      case IDM_800X600:
	m_XWinSize = 800; m_YWinSize = 600;
	break;
      case IDM_1024X768:
	m_XWinSize = 1024; m_YWinSize = 768;
	break;
      case IDM_1024X512:
	m_XWinSize = 1024; m_YWinSize = 512;
	break;
      default:
	m_XWinSize = 320; m_YWinSize = 256;
	break;
...
void BeebWin::ReadDisc(int Drive,HMENU dmenu)
{
  ...
  ofn.nFilterIndex = GetProfileInt("BeebEm", "LoadDiscFilter", 1);
	...
	if (MachineType==0) {
		if (dsd) { Load1770DiscImage(FileName,Drive,1,dmenu); } // 1 = dsd
		if ((!dsd) && (!adfs)) { Load1770DiscImage(FileName,Drive,0,dmenu); } // 0 = ssd
		if (adfs) {
...
void BeebWin::SetWindowAttributes(bool wasFullScreen)
{
  ...
  if (m_isFullScreen)
  {
	...
	{
	  char  errstr[200];
	  sprintf(errstr,"DirectX fail: changing screen size\nFail code %X",
	  				ddrval);
	  MessageBox(m_hWnd,errstr,WindowTitle,MB_OK|MB_ICONERROR);
	}
      }
  ...
  else
  {
    if (m_DirectDrawEnabled && m_DXInit == TRUE)
    {
      ...
      {
	char  errstr[200];
	sprintf(errstr,"DirectX fail: scr size (InitSurfaced)\nErr code %X",
				ddrval);
	MessageBox(m_hWnd,errstr,WindowTitle,MB_OK|MB_ICONERROR);
      }
    }
    ...
    style &= ~WS_POPUP;
    style |= WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX;
// [DMS] Removed the MINIMIZEBOX, as suspect this causes us to lose the window
//    style |= WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU;
    ...
    SetWindowPos(m_hWnd, HWND_TOP, m_XWinPos, m_YWinPos,
			...
			m_YWinSize + (GetSystemMetrics(SM_CYFIXEDFRAME) * 2) +
			(GetSystemMetrics(SM_CYMENUSIZE) * 
				((m_MenuIdWinSize == IDM_160X128) ? 2:1)) +
			GetSystemMetrics(SM_CYCAPTION) + 1,
...
void BeebWin::HandleCommand(int MenuId)
{
  ...
  switch (MenuId)
  {
    case IDM_LDI_COMMS:
      switch (currLvDisc)
      {
        case 0: CheckMenuItem(hMenu, IDM_LDI_COMMS, MF_UNCHECKED); break;
        case 1: CheckMenuItem(hMenu, IDM_LDI_COMMN, MF_UNCHECKED); break;
        case 2: CheckMenuItem(hMenu, IDM_LDI_NATA, MF_UNCHECKED); break;
        case 3: CheckMenuItem(hMenu, IDM_LDI_NATB, MF_UNCHECKED); break;
      }
      CheckMenuItem(hMenu, IDM_LDI_COMMS, MF_CHECKED);
      currLvDisc = 0;
      break;
    case IDM_LDI_COMMN:
      switch (currLvDisc)
      ...
      CheckMenuItem(hMenu, IDM_LDI_COMMN, MF_CHECKED);
      currLvDisc = 1;
      break;
    case IDM_LDI_NATA:
      switch (currLvDisc)
      ...
      CheckMenuItem(hMenu, IDM_LDI_NATA, MF_CHECKED);
      currLvDisc = 2;
      break;
    case IDM_LDI_NATB:
      switch (currLvDisc)
      ...
      CheckMenuItem(hMenu, IDM_LDI_NATB, MF_CHECKED);
      currLvDisc = 3;
      break;
	...
	if( ddrval != DD_OK )
	{
	  ...
	  CheckMenuItem(hMenu, IDM_DD32ONOFF,
	  			m_isDD32 ? MF_CHECKED : MF_UNCHECKED);
      }
      break;
...
//	  SetWindowPlacement(GETHWND,&tWindowPlacement);
    ...
    case IDM_ALLOWWRITES_ROMF:
      CheckMenuItem(hMenu,  MenuId, RomWritable[(MenuId-IDM_ALLOWWRITES_ROM0)] ?
...
/*** case IDM_TUBERESET:
    ...
    case ID_HIDEMENU:
      ...
      CheckMenuItem(hMenu, ID_HIDEMENU, (HideMenuEnabled) ?
    ...
    case ID_TAPESOUND:
      ...
      CheckMenuItem(m_hMenu,ID_TAPESOUND,(TapeSoundEnabled)?
      			MF_CHECKED:MF_UNCHECKED);
      break;
    case ID_HOSTCLOCK:
      UseHostClock=1-UseHostClock;
      CheckMenuItem(m_hMenu,ID_HOSTCLOCK,(UseHostClock)?
      			MF_CHECKED:MF_UNCHECKED);
      SoundReset();
